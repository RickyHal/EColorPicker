...
public class ColorRingPickerView extends BaseColorPickerView {
    ...
    public ColorRingPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void setSelectedBlockColor(@ColorInt int color) {
        if (mSelectedBlockColor == color) return;
    ...
    private void drawTouchBlock(Canvas canvas) {
        ...
        if (mIsRelease || mCurrentColor == -1) return;
    ...
    private void drawScaleMirror(Canvas canvas) {
        if ((mTouchX == 0 && mTouchY == 0) || mIsRelease || mCurrentColor == -1 || !mHasScaleMirror) {
            return;
        }
        float cx = mTouchX + mMirrorRadius;
        float cy = mTouchY - mMirrorRadius;

        if (cx < mMirrorRadius) {
            cx = mMirrorRadius;
        }
        if (cx > getWidth() - mMirrorRadius) {
            cx = getWidth() - mMirrorRadius;
        }

        if (cy < mMirrorRadius) {
            cy = mMirrorRadius;
        }
        if (cy > getHeight() - mMirrorRadius) {
            cy = getHeight() - mMirrorRadius;
        }
        mPaint.setStyle(Style.FILL);
        mPaint.setColor(mCurrentColor);
        canvas.drawCircle(cx, cy, mMirrorRadius, mPaint);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int defaultWidth = (int) getDpValue(DEFAULT_WIDTH_DP);
        int defaultHeight = (int) getDpValue(DEFAULT_HEIGHT_DP);
